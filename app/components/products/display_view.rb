          ...
          div(class: "px-6 pb-4") do
            ...
            if current_library_items.any?
                  ...
                  current_library_items.each do |item|
                    div(class: "flex justify-between items-center text-sm") do
                      ...
                      div(class: "flex items-center gap-2") do
                        if item.want?
                          span(class: "text-blue-600 text-xs bg-blue-100 px-2 py-1 rounded") { "Want" }
                        end
                        if item.condition.present?
                          span(class: "text-green-600 text-xs bg-green-100 px-2 py-1 rounded") { item.condition.name }
                        end
                      end
                    end
                  end
              ...
              div(class: "bg-white border border-blue-200 rounded-lg p-4 text-center") do
                p(class: "text-sm text-blue-700") { "Not in any library yet" }
              end
            end
          end
