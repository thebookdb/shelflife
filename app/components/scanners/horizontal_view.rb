            ...
            div(
              ...
              div do
                ...
                select(
                  ...
                  option(value: "", selected: !Current.library) { "None (scanned list)" }
            ...
            if @recent_items.any?
              div(class: "flex-1") do
                ...
                div(class: "space-y-2 max-h-64 overflow-y-auto") do
                  @recent_items.each do |library_item|
                    div(class: "bg-white border rounded-lg p-3 shadow-sm") do
                      if library_item.product
                        div(class: "flex items-center gap-2") do
                          div(class: "text-xs text-gray-500") { library_item.date_added.strftime("%H:%M") }
                          div(class: "flex-1 text-sm font-medium text-gray-800 truncate") do
                            library_item.product.title || library_item.product.gtin
                          end
                        end
                      else
