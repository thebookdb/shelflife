...
export default class extends Controller {
  static targets = ["scanner", "result", "placeholder", "startButton", "stopButton", "librarySelect", "scanResult", "libraryStatus", "orientationDisplay"]
  static values = { 
  ...
  connect() {
    console.log("Adaptive barcode scanner controller connected", { scannerPage: this.scannerPageValue })
  ...
  updateOrientationDisplay() {
    if (this.hasOrientationDisplayTarget) {
      ...
      const text = this.currentOrientationValue === "landscape" ? "Landscape Mode" : "Portrait Mode"
  ...
  initializeScanner() {
    console.log("Initializing adaptive scanner on element:", this.scannerTarget.id, this.scannerTarget)
  ...
  stopScanning() {
    ...
    console.log("Stopping adaptive scanner...")
    ...
    if (this.scanner) {
      this.scanner.clear().catch(error => {
        console.error("Failed to clear adaptive scanner:", error)
      })
      this.scanner = null
    }
  }
