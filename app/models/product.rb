class Product < ApplicationRecord
  ...
  after_update :broadcast_product_updates, if: :saved_change_to_tbdb_data?
  ...
  # Broadcast updates when product data changes (e.g. enrichment from TBDB)
  def broadcast_product_updates
    # Refresh the product show page
    Turbo::StreamsChannel.broadcast_refresh_to("product_#{id}")

    # Refresh any library pages that contain this product
    libraries.distinct.each do |library|
